  ...
  return (
    <div className="p-6 pt-12 xl:px-12 2xl:px-20 w-full mx-auto h-full overflow-y-scroll">
      ...
      {images.length > 0 ? (
        <div className="flex flex-wrap max-sm:justify-center gap-5">
          {images.map((item, index) => (
            <a
              ...
              href={item.imageUrl}
              target="_blank"
              className="relative block rounded-lg overflow-hidden border border-gray-200 dark:border-purple-700 shadow-sm hover:shadow-md transition-shadow duration-300 group"
            >
              <img
                src={item.imageUrl}
                alt=""
        ...
        <p className="text-gray-500 dark:text-gray-300">No images found.</p>
      )}
    </div>
  )
}
